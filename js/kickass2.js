...
(function(window) {
  ...
  var GameGlobals = {
    ...
    path: function() {
      return "https://kickassapp.com/" + Array.prototype.slice.call(arguments)
          .join("");
    },
    hasCanvas: (typeof document.createElement('canvas')
  ...
  var Menu = new Class({
    ...
    generate: function(parent) {
      ...
      var shipId = getGlobalNamespace()
          .KICKASSSHIPID || "";
      this.url = GameGlobals.path('intermediate_postmessage.html?url=' +
          encodeURIComponent(getGlobalNamespace()
              .KICKASSURL || document.location.href) + "&origin=" + encodeURIComponent(document.location.href) + "&preship=" + (shipId) + "&is_campaign=" + (this.game.isCampaign() ? "true" : "") + "&is_mysite=" + (this.game.isMySite() ? "true" : ""));
    ...
    onGameReady: function() {
      ...
      this.socketPostMessage("url:!" + (getGlobalNamespace()
          .KICKASSURL || document.location.href));
    ...
    messageTypeChangeShip: function(pieces) {
      ...
      if (isInitial && getGlobalNamespace()
          .KICKASSSHIP) {
        return;
      }
      this.shipId = shipId;
  ...
  var MenuManager = new Class({
    ...
    getShareHTML: function() {
      if (typeof getGlobalNamespace()
          .KICKASS_SHARE_URL !== "undefined") {
        if (getGlobalNamespace()
            .KICKASS_SHARE_URL) {
          var url = encodeURIComponent(getGlobalNamespace()
              .KICKASS_SHARE_URL);
  ...
  var AudioManager = new Class({
    initialize: function(src, formats) {
      ...
      channels = 8;
      this.supportsAudio = (typeof document.createElement('audio')
          .play) != 'undefined';
  ...
  var Player = new Class({
    ...
    update: function(tdelta) {
      if (this.isBroken) {
        ...
        for (var i = 0; i < this.lineOffsets.length; i++) {
          this.lineOffsets[i].pos.add(this.lineOffsets[i].dir.cp()
              .setLength(50)
              .mul(tdelta));
        }
        this.sheet.clear();
      ...
      var frictionedAcc = this.acc.mulNew(tdelta)
    ...
    activateThrusters: function() {
      ...
      this.acc = (new Vector(500, 0))
          .setAngle(this.dir.angle());
    },
    ...
    getSizeFromVertsAndObjects: function() {
      ...
      for (var i = 0, vert; vert = this.verts[i]; i++)
        largestDistance = Math.max(largestDistance, (new Vector(vert))
            .len());
      for (var i = 0, obj; obj = this.thrusters[i]; i++) {
        var p1 = (new Vector(obj.pos.x - obj.size.width / 2, obj.pos.y - obj.size.height / 2))
            .rotate(obj.angle);
        var p2 = (new Vector(obj.pos.x + obj.size.width / 2, obj.pos.y - obj.size.height / 2))
            .rotate(obj.angle);
        var p3 = (new Vector(obj.pos.x - obj.size.width / 2, obj.pos.y + obj.size.height / 2))
            .rotate(obj.angle);
        var p4 = (new Vector(obj.pos.x + obj.size.width / 2, obj.pos.y + obj.size.height / 2))
            .rotate(obj.angle);
  ...
  var BulletManager = new Class({
    ...
    updateEnemyIndex: function() {
      ...
      for (var i = 0, el; el = all[i]; i++) {
        if (this.isDestroyable(el)) {
          ...
          el.KICKASSOLDBORDER = el.style.outline || (document.defaultView.getComputedStyle(el, null)
              .outline);
        }
      }
      this.nextUpdate = this.updateDelay;
    ...
    isDestroyable: function(element, ignoreSize) {
      ...
      for (var i = 0, child; child = element.childNodes[i]; i++) {
        if (child.nodeType === 1 && ELEMENTSTHATARENOTTOBEINCLUDED.indexOf(child.tagName) === -1 && (child.offsetWidth >= ELEMENTSIZETHRESHOLD && child.offsetHeight >= ELEMENTSIZETHRESHOLD) && document.defaultView.getComputedStyle(child, null)
            .visibility !== 'hidden') {
          return false;
        }
      }
  ...
  var Cannon = new Class({
    ...
    checkCollision: function(bullet) {
      ...
      this.game.menuManager.addPoints(Math.min(hit.getElementsByTagName('*')
          .length + 1, 100), bullet.pos);
      if (!hit.isShot) {
    ...
    createBullet: function(bulletClass) {
      ...
      bullet.manager = this;
      bullet.initCanvas();
      bullet.vel.add(bullet.vel.cp()
          .setLength(this.player.vel.len()));
      return bullet;
    },
    getABulletPos: function() {
      return this.player.pos.cp()
          .add(this.pos.cp()
              .rotate(this.player.dir.angle() + Math.PI / 2));
    },
    getABulletDir: function() {
      return this.player.dir.cp()
          .rotate(this.angle);
    },
    destroy: function() {}
  });
  ...
  var Bullet = new Class({
    ...
    update: function(tdelta) {
      this.pos.add(this.vel.setAngle(this.dir.angle())
          .mulNew(tdelta));
      this.checkBounds();
      this.draw();
    },
  ...
  var ParticleExplosion = new Class({
    ...
    update: function(tdelta) {
      for (var i = 0, particle; particle = this.particles[i]; i++)
        particle.pos.add(particle.vel.mulNew(tdelta)
            .mul(random(0.5, 1.0))
    ...
    generateParticles: function() {
      for (var i = 0, j = !GameGlobals.hasCanvas ? 10 : 40; i < j; i++) {
        this.particles.push({
          dir: (new Vector(random(0, 20) - 10, random(0, 20) - 10))
              .normalize(),
